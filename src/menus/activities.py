...
def activities_menu(player):
    ...
    while not selected:
        ...
        if choice == _("Back"):
                    ...
                    if len(player.illnesses) == 0:
                        ...
                        if choice > 1:
                            ...
                            if illness == "Depression":
                                ...
                                was_cured = player.stress < randint(50, 80) and one_in(
                                    3
                                )
                                if was_cured:
                                    player.change_health(randint(4, 8))
                                ...
                                was_cured = (
                                    one_in(20) and randint(1, 100) <= player.health
                                )
                                if was_cured:
            ...
            if yes_no(
                ...
                if player.money < fee:
                    ...
                    if health_mod > 0 and player.illnesses and x_in_y(health_mod, 200):
                        ...
                        display_event(
                            _("You are no longer suffering from {illness}.").format(
                                illness=illness
                            )
                        )
                        player.remove_illness(illness)
                        ...
                        player.die(
                            _("You died after taking a witch doctor's treatments.")
                        )
            ...
            choices = [_("Back")]
            if player.age >= 12:
                choices.extend([_("Meditate"), _("Gym"), _("Library")])
            ...
            if choice == _("Meditate"):
                ...
                if not player.meditated:  # You can only get the bonus once per year
                    ...
                    if player.times_meditated == 0 or one_in(20):
                        ...
                        if (
                            player.times_meditated > 0
                            and x_in_y(2, 5)
                            and not x_in_y(player.stress, 100)
                        ):
                ...
                display_bar(_("Your Enjoyment"), enjoyment)
                if (
                    not player.visited_library
                ):  # You can only get the bonus once per year
                    ...
                    if (
                        one_in(48)
                        and random.random() < 0.95**player.times_visited_library
                    ):
                ...
                while not done:
                    ...
                    if choice <= len(names):
                        ...
                        if player.has_trait("BOOK_LOVER"):
                            if happy_amount > 0:
                                happy_amount = math.ceil(
                                    min(happy_amount * 1.5, (100 + happy_amount) / 2)
                                )
                            else:
                        ...
                        while reading:
                            clear_screen()
                            print(
                                _('You are reading "{book_title}"').format(
                                    book_title=title
                                )
                            )
                            ...
                            if choice == 1:
                                ...
                                if pages >= total_pages:
                                    ...
                                    print(
                                        _(
                                            'You read all {pages} pages of "{book}".'
                                        ).format(pages=total_pages, book=title)
                                    )
                                    done = True
                                    reading = False
                                    ...
                                    if happy_amount > 0:
                                        ...
                                        enjoyment = round_stochastic(
                                            enjoyment + (100 - enjoyment) * r
                                        )
                                    display_bar(_("Your Enjoyment"), enjoyment)
                                    learn = []
                                    if total_pages > randint(48, 150) and x_in_y(
                                        math.sqrt(smarts_amount), 100
                                    ):
                                        learn.append("NERD")
                                    ...
                                    if happy_amount > 0:
                                        amount = (
                                            random.uniform(0, math.sqrt(happy_amount))
                                            * (min(player.stress / 70, 1)) ** 2
                                        )
            ...
            if player.age - player.last_plastic_surgery < 6:
                ...
                if player.money < 25000:
                    ...
                    names = list(
                        map(lambda s: _("Dr. {name}").format(name=s[0]), surgeons)
                    )
                    options = [
                        names[i]
                        + "\n   "
                        + _("Reputation: {bar}").format(bar=get_bar(surgeons[i][1]))
                    ...
                    if one_in(5) and not x_in_y(reputation, 100):
                        ...
                        print(
                            _("Your plastic surgery was botched by {name}!").format(
                                name=name
                            )
                        )
                        ...
                        if (
                            player.health <= 0
                            and one_in(5)
                            and not x_in_y(player.karma, 100)
                            and damage >= randint(20, 100)
                        ):
                            player.die(
                                _("You died after receiving a botched plastic surgery.")
                            )
                    else:
