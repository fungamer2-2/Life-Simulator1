...
class Player(Person):
    ...
    def __init__(self, first=None, last=None, gender=None):
        ...
        if (
            manual and "belle" in first.lower()
        ):  # Congratulations, you discovered an easter egg!
            looks = 100
    ...
    def process_relation_death(self, relation, reason=None):
        ...
        if self.has_trait("THANATOPHOBIA"):
            ...
            display_event(
                _(
                    "You feel intense panic as the death of your loved one reminds you of how much you are afraid of death."
                )
            )
        ...
        if self.age >= randint(12, 17) and self.has_trait("THANATOPHOBIA"):
            if one_in(10):
                thanatophobe_msgs = [
                    ...
                    _(
                        "You feel extremely anxious at the thought of one day not being here anyone."
                    ),
                    _(
                        "You're afraid to die. You feel you must stay out of any situations that involve dying."
                    ),
                    _(
                        "The thought of dying makes your heart race and you feel anxious."
                    ),
                    _(
                        "You find it hard to sleep at night, afraid of even the thought of dying."
                    ),
    ...
    def random_events(self):
        ...
        if (
            self.partner
            and self.money >= 1000000
            and not x_in_y(self.karma, 130)
            and one_in(250)
        ):
            ...
            print(
                _(
                    "Your {partner} has been kidnapped and is being held for ${ransom} ransom!"
                ).format(partner=rel, ransom=ransom)
            )
            print(_("The kidnappers warn you not to involve the police."))
            print(_("What will you do?"))
            self.change_happiness(-randint(30, 40))
            choice = choice_input(
                _("Pay the ransom"), _("Involve the police"), _("Do nothing")
            )
            success = False
            if choice == 1:
                ...
                if success:
                    display_event(
                        _(
                            "You successfully paid ${ransom} ransom in exchange for {name}'s release."
                        ).format(ransom=ransom, name=self.partner.firstname),
                        cls=False,
                    )
                else:
                    display_event(
                        _(
                            "You paid ${ransom} ransom but they did not return {name}!"
                        ).format(ransom=ransom, name=self.partner.firstname),
                        cls=False,
                    )
            elif choice == 2:
                ...
                if success:
                    display_event(
                        _(
                            "The police managed to secure {name}'s release and arrest the kidnappers!"
                        ).format(name=self.partner.firstname),
                        cls=False,
                    )
                else:
                    display_event(
                        _("The police were unable to secure {name}'s release!").format(
                            name=self.partner.firstname
                        ),
                        cls=False,
                    )
            else:
            ...
            if success:
                ...
                display_event(
                    _("{name} and {his_her} kidnappers vanished!").format(
                        name=self.partner.firstname, his_her=self.partner.his_her()
                    )
                )
                self.process_relation_death(
                    self.partner, reason=_("is presumed dead after disappearing")
                )
        elif (
            self.age >= 18
