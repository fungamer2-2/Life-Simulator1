...
class Child(Relationship):
	...
	def age_up(self):
		...
		if self.is_in_uv > 0:
			self.is_in_uv -= 1
			if self.is_in_uv == 0:
				print(_("Your {child} graduated from university.").format(child=self.name_accusative()))
				self.change_smarts(randint(6, 12))
				self.change_happiness(randint(8, 14))
		if self.age == 6:
			print(_("Your {child} is starting elementary school").format(child=self.name_accusative()))
			self.change_smarts(randint(0, 2))
		if self.age == 12:
			print(_("Your {child} is starting middle school").format(child=self.name_accusative()))
			self.change_smarts(randint(0, 3))
		if self.age == 14:
			print(_("Your {child} is starting high school").format(child=self.name_accusative()))
			self.change_smarts(randint(0, 4))
		if self.age == 17:
			print(_("Your {child} graduated from high school").format(child=self.name_accusative()))
			...
			if self.partner.death_check():
				...
				if (self.may_divorce or self.partner.status != 2) and randint(1, breakup_chance) == 1:
					...
					if self.partner.status == 2:
						...
						print(_("Your {child}, and {partner} broke up.").format(child=self.name_accusative(), partner=self.partner.name))
			...
			if self.age >= 18 and randint(1, 700) <= self.partner_desire:
				...
				for i in range(attempts):
					...
					if p.compatibility_check(self):
						self.partner = p
						self.change_happiness(randint(5, 20))
						print(_("Your {child} is now in a relationship with {partner}, age {age}.").format(child=self.name_accusative(), partner=p.name, age=p.age))
						break